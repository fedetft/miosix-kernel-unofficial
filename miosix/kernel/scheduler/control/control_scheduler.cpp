...
static bool threadWokenInRound = false; // flag for regulator
static bool threadWokenInBurst = false; // flag for IRQfindNextThread
...
unsigned int ControlScheduler::IRQfindNextThread()
{
    ...
    if(cur!=idle)
    {
        ...
        if (threadWokenInBurst)
        {
            ...
            // Burst Correction - re-assign the remainder of round-time to all
            // ready threads including the one that has just woke up (t)
            int remTr = SP_Tr - Tr;
            int tot = remTr;
            // Assign burst to woken threads from remainder of the round
            for(Thread *it=threadList;it!=0;it=it->schedData.next)
            {
                if (it->schedData.wokenDuringBurst)
                {
                    it->schedData.wokenDuringBurst = false;
                    it->schedData.bo = it->schedData.alfa * remTr;
                    tot += it->schedData.bo;
                }
            }
            // Normalize bursts
            float nrmFact = (float)remTr / tot;
            // TODO - USE activeThreads list since bo will become zero for non-ready
            for(Thread *it=threadList;it!=0;it=it->schedData.next)
            {
                it->schedData.bo = (it->schedData.bo - it->schedData.Tp) * nrmFact
                    +it->schedData.Tp;
            }
        }
    }
...
void ControlScheduler::IRQwaitStatusHook(Thread* t)
{
    ...
    if (t->flags.isReady()){
        ...
        threadWokenInBurst = true; // Signal for IRQfindNextThread
...
void ControlScheduler::IRQrunRegulator(bool allReadyThreadsSaturated)
{
    if(reinitRegulator)
    {
        ...
        for(Thread *it=threadList;it!=0;it=it->schedData.next)
        {
            ...
            // Apply Saturations
            it->schedData.bo=min(max(it->schedData.SP_Tp,bMin),bMax);
        ...
        if (threadWokenInRound){
            // bc = sum(threads.Tp)/sum(threads.alfa(threads.ready==1)) - sched.Tr
            int sTp = 0;
            float sAlfa = 0;
            for(Thread *it=threadList;it!=0;it=it->schedData.next)
            {
        ...
        for(Thread *it=threadList;it!=0;it=it->schedData.next)
        {
            if (it->flags.isReady())
            {
                it->schedData.SP_Tp=static_cast<int>(it->schedData.alfa*nextRoundTime);
                //saturation
                it->schedData.bo=min(max(it->schedData.SP_Tp,bMin),bMax);
            } else {
