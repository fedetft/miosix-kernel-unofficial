...
public:
     ...
     * Constructor
     */
    Cc2520(SPI& spi, miosix::GpioPin cs, miosix::GpioPin reset, miosix::GpioPin vreg);
     ...
     */
    int writeFrame(unsigned char length, const void* frame);
...
//     int writeFrame(const Frame& frame);
     ...
     */
    int readFrame(unsigned char& lenght, unsigned char* pframe);
...
//     int readFrame(Frame& frame);
    ...
    void disableWakeupManagement() { noWakeupManagement=true; }
...
private:
     ...
     */
    bool writeReg(Cc2520FREG reg, unsigned char data);
     ...
     */
    bool writeReg(Cc2520FREG reg, const unsigned char *data, int len);
     ...
     * \param data data to write
     */
    bool writeReg(Cc2520SREG reg, unsigned char data);
     ...
     * \param len data length
     */
    bool writeReg(Cc2520SREG reg, const unsigned char *data, int len);
     ...
     */
    bool readReg(Cc2520FREG reg, unsigned char& result);
     ...
     * \return true if the operation is successful, false otherwise
     */
    bool readReg(Cc2520SREG reg, unsigned char& result);
     ...
     * \return status byte
     */
    unsigned char commandStrobe(Cc2520SpiIsa ins);
    ...
    bool isExcRaised(Cc2520Exc0 exc,
                     unsigned char status = CC2520_STATUS_EXC_A);
