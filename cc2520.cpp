...
Cc2520::Cc2520(SPI& spi, miosix::GpioPin cs, miosix::GpioPin reset, miosix::GpioPin vreg)
    :  autoFCS(true), noWakeupManagement(false), mode(DEEP_SLEEP), spi(spi), cs(cs), reset(reset), vreg(vreg)
{
//     cc2520GpioInit();  //see HW_mapping.h
// 
//     this->cs.mode(miosix::Mode::OUTPUT);
    this->cs.high();
// 
//     this->vreg.mode(miosix::Mode::OUTPUT);
    this->vreg.low();
// 
//     this->reset.mode(miosix::Mode::OUTPUT);
    this->reset.high();
        ...
        case DEEP_SLEEP:
            switch(this->mode)
            {
                ...
                case IDLE:
                    ...
                    reset.low(); //take low for 0.1ms
//                     usleep(500); //By TFT: 18/09/2015: why do we need this delay??
                    cs.high();
                    vreg.low();
                    reset.high();
                    break;
...
void Cc2520::setShortAddress(const unsigned char address[2])
{
    ...
    //little endian
    spi.sendRecv(CC2520_INS_MEMORY_WRITE | 0x3);
    spi.sendRecv(0xf4);
    spi.sendRecv(address[1]);
    spi.sendRecv(CC2520_INS_MEMORY_WRITE | 0x3);
    spi.sendRecv(0xf5);
    spi.sendRecv(address[0]);
...
void Cc2520::setPanId(const unsigned char panId[])
{
    ...
    if (this->mode != SLEEP  && this->mode != DEEP_SLEEP)
    {
        ...
        spi.sendRecv(CC2520_INS_MEMORY_WRITE | 0x3);
        spi.sendRecv(0xf2);
        spi.sendRecv(panId[1]);
        spi.sendRecv(CC2520_INS_MEMORY_WRITE | 0x3);
        spi.sendRecv(0xf3);
        spi.sendRecv(panId[0]);
...
int Cc2520::writeFrame(unsigned char length, const void* frame)
{
    const unsigned char *pframe = reinterpret_cast<const unsigned char *>(frame);
    ...
    cs.low();
    delay();
    unsigned char status = spi.sendRecv(CC2520_INS_TXBUF);
    spi.sendRecv(length+autoFCS*SIZE_AUTO_FCS); //adding 2 bytes of FCS
    for(int i=0; i<length; i++) spi.sendRecv(pframe[i]);
    cs.high();
    delay();
...
int Cc2520::readFrame(unsigned char& length, unsigned char* pframe)
{
    ...
    cs.low();
    delay();
    unsigned char status = spi.sendRecv(CC2520_INS_RXBUF);
    currLen = spi.sendRecv()-2*autoFCS;
    if(currLen>length)
    {
        for(int i=0; i<currLen; i++)
            (currLen<=length) ? pframe[i]=spi.sendRecv()
                                                : spi.sendRecv();
        result = 0;
    }else
    {
        for(int i=0; i<currLen; i++) pframe[i]=spi.sendRecv();
        length = currLen;
        result = 1;
    }
...
// int Cc2520::readFrame(Frame& frame)
// {
//     if(this->mode != RX) return -3;
...
// 
//     unsigned char status = spi.sendRecv(CC2520_INS_RXBUF);
//     if(!frame.initFrame(spi.sendRecv()))
//     {
//         cs.high();
//         delay();
//         flushRxFifoBuffer();
...
int Cc2520::isTxFrameDone()
{
    if(this->mode != TX) return -1;
    return isExcRaised(CC2520_EXC_TX_FRM_DONE);
}

int Cc2520::isRxFrameDone()
{
...
int Cc2520::isRxBufferNotEmpty()
{
    if(this->mode != RX) return -1;
//     if(cc2520::fifo_irq::value()==0 && cc2520::fifop_irq::value()==1)
    if(transceiver::gpio1::value()==0 && transceiver::gpio2::value()==1)
    {
        #if CC2520_DEBUG >1
           commandStrobe(CC2520_INS_SFLUSHRX);
           printf("--CC2520_DEBUG-- Buffer overflow!\n");
...
//     }else if(cc2520::fifo_irq::value()==1 || cc2520::fifop_irq::value()==1)
    }else if(transceiver::gpio1::value()==1 || transceiver::gpio2::value()==1)
    {
        return true;
    }
    return false;
...
bool Cc2520::writeReg(Cc2520FREG reg, unsigned char data)
{
    if(reg>0x3F) return false;
    cs.low();
    delay();
    spi.sendRecv(CC2520_INS_REGISTER_WRITE | reg);
    spi.sendRecv(data);
    cs.high();
    delay();
    return true;
...
bool Cc2520::writeReg(Cc2520SREG reg, unsigned char data)
{
    if(reg>0x7E) return false;
    cs.low();
    delay();
    spi.sendRecv(CC2520_INS_MEMORY_WRITE);
    spi.sendRecv(reg);
    spi.sendRecv(data);
    cs.high();
    delay();
...
bool Cc2520::readReg(Cc2520FREG reg, unsigned char& result)
{
    if(reg>0x3F) return false;
    cs.low();
    delay();
    spi.sendRecv(CC2520_INS_REGISTER_READ | reg);
    result =spi.sendRecv();
    cs.high();
    delay();
    return true;
 ...
 bool Cc2520::readReg(Cc2520SREG reg, unsigned char& result)
{
    if(reg>0x7E) return false;
    cs.low();
    delay();
    spi.sendRecv(CC2520_INS_MEMORY_READ);
    spi.sendRecv(reg);
    result =spi.sendRecv();
    cs.high();
    delay();
...
unsigned char Cc2520::readStatus()
{
    cs.low();
    delay();
    unsigned char result=spi.sendRecv(CC2520_INS_SNOP); //NOP command
    cs.high();
    delay();
    return result;
}

unsigned char Cc2520::commandStrobe(Cc2520SpiIsa ins)
{
    unsigned char status;
    cs.low();
    delay();
    status = spi.sendRecv(ins);
    cs.high();
    delay();
    return status;
}

bool Cc2520::isExcRaised(Cc2520Exc0 exc, unsigned char status)
{
            ...
            ((status & CC2520_STATUS_EXC_B) == CC2520_STATUS_EXC_B))
    {
        ...
        cs.low();
        delay();
        spi.sendRecv(CC2520_INS_REGISTER_READ | CC2520_EXCFLAG0);
        result =spi.sendRecv();
        cs.high();
        delay();
        ...
        if((result & exc )== exc)
        {
            ...
            cs.low();
            delay();
            spi.sendRecv(CC2520_INS_REGISTER_WRITE | CC2520_EXCFLAG0);
            spi.sendRecv(~exc);
            cs.high();
            delay();
            return true;
...
bool Cc2520::isExcRaised(Cc2520Exc1 exc, unsigned char status)
{
    if((status & CC2520_STATUS_EXC_A) == CC2520_STATUS_EXC_A ||
            ((status & CC2520_STATUS_EXC_B) == CC2520_STATUS_EXC_B))
    {
        ...
        cs.low();
        delay();
        spi.sendRecv(CC2520_INS_REGISTER_READ | CC2520_EXCFLAG1);
        result =spi.sendRecv();
        cs.high();
        delay();
        ...
        if((result & exc )== exc)
        {
            ...
            cs.low();
            delay();
            spi.sendRecv(CC2520_INS_REGISTER_WRITE | CC2520_EXCFLAG1);
            spi.sendRecv(~exc);
            cs.high();
            delay();
            return true;
...
bool Cc2520::isExcRaised(Cc2520Exc2 exc, unsigned char status)
{
    if((status & CC2520_STATUS_EXC_A) == CC2520_STATUS_EXC_A ||
            ((status & CC2520_STATUS_EXC_B) == CC2520_STATUS_EXC_B))
    {
        ...
        cs.low();
        delay();
        spi.sendRecv(CC2520_INS_REGISTER_READ | CC2520_EXCFLAG2);
        result =spi.sendRecv();
        cs.high();
        delay();
        ...
        if((result & exc )== exc)
        {
            ...
            cs.low();
            delay();
            spi.sendRecv(CC2520_INS_REGISTER_WRITE | CC2520_EXCFLAG2);
            spi.sendRecv(~exc);
            cs.high();
            delay();
            return true;
...
//     while(!xoscInterrupt)
//     {
//         //By TFT 1/4/2015: if miso is already high, don't wait forever
//         if(cc2520::miso::value()) break;
//         
...
{
    writeReg(CC2520_TXPOWER,power); //TX power 0 dBm
}